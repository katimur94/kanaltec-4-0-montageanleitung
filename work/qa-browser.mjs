...
await page.locator('#millingView').click();
for(const [name,t] of [['intrusion',0],['trim',.25],['round',.95],['outer',.55],['inner',1.6],['exchange',2.8],['position',3.6],['filling',7.78],['fill',7.95],['done',9.99],['rewind',.55]]){
 await page.locator('#timeline').evaluate((el,t)=>{el.value=t*1000;el.dispatchEvent(new Event('input',{bubbles:true}));},t);
 await page.waitForTimeout(350);
 await page.locator('#viewport').screenshot({path:`work/qa/${name}.png`});
 console.log(name,await page.locator('#viewport').evaluate(el=>({...el.dataset})));
}
await page.locator('#timeline').evaluate(el=>{el.value=9999;el.dispatchEvent(new Event('input',{bubbles:true}));});await page.waitForTimeout(300);
...
for(const [name,t] of [['injection-early',7.52],['injection-wall',7.78],['injection-full',7.95]]){await page.locator('#timeline').evaluate((el,t)=>{el.value=t*1000;el.dispatchEvent(new Event('input',{bubbles:true}));},t);await page.waitForTimeout(300);await page.locator('#viewport').screenshot({path:`work/qa/${name}.png`});}
...
await page.locator('#timeline').evaluate(el=>{el.value=7520;el.dispatchEvent(new Event('input',{bubbles:true}));});await page.waitForTimeout(300);await page.locator('#viewport').screenshot({path:'work/qa/ground-early.png'});
await page.locator('#millingView').click();
for(const dn of ['300','700']){await page.locator('#family').selectOption(dn);await page.locator('button[data-stage="1"]').click();await page.waitForTimeout(350);await page.locator('#viewport').screenshot({path:`work/qa/inner-${dn}.png`});}
