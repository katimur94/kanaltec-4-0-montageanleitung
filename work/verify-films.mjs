...
await page.goto(live?'https://katimur94.github.io/kanaltec-4-0-montageanleitung/Videos/DSS-Flex-Verfahren-2026/Videos-ansehen.html':pathToFileURL(path.resolve(output,'Videos-ansehen.html')).href);
...
for(const video of await page.locator('video:not([src*="verschliessen"])').all()){
 const result=await video.evaluate(async v=>{
  v.muted=true;v.currentTime=69;await new Promise(resolve=>v.addEventListener('seeked',resolve,{once:true}));
  await v.play();await new Promise(resolve=>setTimeout(resolve,500));v.pause();
  return {file:v.getAttribute('src'),width:v.videoWidth,height:v.videoHeight,duration:v.duration,advanced:v.currentTime>69.1,error:v.error?.message||null};
 });results.push(result);
}
await page.screenshot({path:`work/qa/video/gallery-${live?'live':'local'}.png`,fullPage:true});await browser.close();
await writeFile(`work/qa/video/browser-${live?'live':'local'}.json`,JSON.stringify({results,errors},null,2));
