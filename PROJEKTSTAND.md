...
**Sanierungsanhänger und neue Videos (23.09.2026):** Eigenständige Seite `DSS-Flex-Sanierungsanhaenger.html` (Quellen in `anhaenger/`, Build `node work/build-anhaenger.mjs`, Fotos der bestehenden Anlage in `anhaenger/Bilder von alte Anlage`): Tandem-Kofferanhänger 3,5 t nur mit DSS-Flex-Komponenten, alles Bewegliche bedienbar, Begehen durch die Seitentür, echte Schalungen DN 300/400 aus `src/model.js` im Schwerlastauszug, Beladungs-/Stützlastrechner. Button **Sanierungsanhänger** in der Kopfzeile der Animation. Videogalerie mit Web-Fassungen der realistischen Verfahrensfilme (drei Formate, mit und ohne Sprecher) und dem Rundgang-Film des Anhängers (`node work/publish-web-films.py`, Masters liegen lokal außerhalb des Repositorys; Film: `node work/render-anhaenger-film.mjs`). Gerätemassen sind Schätzungen, Aufbau/Achslage mit dem Fahrzeugbauer abstimmen. Mit Commit `0469f3d` auf GitHub und Pages veröffentlicht, Live-Seite, Button und alle Galerie-Videos geprüft. Details in [FORTSCHRITT.md](FORTSCHRITT.md).

**Realismus-Überarbeitung (23.09.2026):** Neuer prozeduraler Wurzeleinwuchs (`src/roots.js`): Wurzeln im Erdreich laufen zum ausgewaschenen Hohlraum *um* den Anschluss und hängen durch den Spalt zwischen Anschlussrohr und Bruchkante als verzweigte Wurzelmatten mit Feinwurzeln in den Kanal – nach Nutzerkorrektur nicht aus der Anschlussöffnung. Der Fräser schneidet sie von unten ab (Shader-Schnitt, auch beim Rückspringen korrekt). Fräsgut (`src/debris.js`): Scherben und abgeschnittene Wurzelbüschel fallen ins Sohlwasser und treiben ab, dazu feiner Staub. Wenig Abwasser in der Sohle (4 % DN) auch in der Web-Präsentation. Infiltration mit physikalisch fallenden Tropfen in Echtzeit, Tropfenbildung an der Kante, Spritzern, Wellenringen, glänzenden Nassspuren, Kalksinter und Tropfsteinen. Nutzerkorrektur zur Endfläche der offenen Variante: etwas ovaler, ca. 265 × 185 mm in der Rohrabwicklung statt 240 × 210 mm, mit unregelmäßigem Rand und auslaufendem, herausgedrücktem Mörtelfilm; Test entsprechend angepasst (Seitenverhältnis 1,3–1,7). Geschlossene Varianten unverändert. Nahtlose Rohr-/Mörtel-/Bodentexturen ohne Blockraster; Rohrschnittflächen jetzt tonfarben statt schwarz (Material erwartete fehlende Vertexfarben). Alle Maße und Schadensbilder sind Darstellungsparameter. `npm test` und Build bestanden. Filmstudio und Videos siehe `work/VIDEO-PRODUKTION.md`.

**Mit Blase ebenfalls runder (20.09.2026):** Auch die offene Anschlussvariante erhält auf Nutzerkorrektur eine kompakte, nur leicht ovale Reparaturfläche: 240 × 210 mm in der Rohrabwicklung statt ungefähr 243 × 134 mm. Anschlussöffnung und beide vollständigen Abdrücke bleiben frei beziehungsweise eingeschlossen; Fräsbahn und Rohrwand stimmen mit der neuen Kontur überein. Blasenmechanik und Füllhöhe unverändert. Geometrie und Bewegungsabläufe für alle fünf DN geprüft, `npm ci`, `npm test` und portabler Build bestanden. Ersetzt die weiter unten beschriebene schmale Form dieser Variante. Alle sechs Videos in den drei Formaten neu gerendert, vollständig decodiert und im Browser geprüft; Hochformat-Kanalblick zeigt die gesamte Kontur. Mit Commit `7af507b` auf Pages veröffentlicht; Build, Live-Modell und alle sechs Live-Videos ohne Fehler geprüft. Details siehe neuesten Eintrag in [FORTSCHRITT.md](FORTSCHRITT.md).

**Rundere Verschlussfläche (20.09.2026):** Neue Nutzerkorrektur für Loch und stillgelegten Anschluss: kompakt und nur leicht oval. Kontur in der Rohrabwicklung jetzt 260 × 230 mm statt 295 × 150 mm; passt über alle fünf DN innerhalb der Schalung und umfasst Schaden sowie vollständige Zulauf-/Sensorabdrücke. Rohrwand, Gussfläche, Schnittkante und optionale Fräsbahn verwenden dieselbe Kontur. Zulauf bleibt mittig, Anschlussfüllung nahezu vollständig. Beide Verschlussfilme neu gerendert, vollständig decodiert und im Browser geprüft; korrigierte Fassungen über Telegram bestätigt zugestellt. Korrektur mit Commit `3bf166c` auf Pages veröffentlicht, erfolgreicher Build und Live-Prüfung von Modell und beiden Filmen. Darstellungsmaße nach Nutzerkorrektur, keine Herstellerangabe. Siehe [SCHADSTELLEN-QUELLEN.md](SCHADSTELLEN-QUELLEN.md) und neuesten Eintrag in [FORTSCHRITT.md](FORTSCHRITT.md).
...
| `src/roots.js` | Prozeduraler Wurzeleinwuchs: verzweigter Wurzelzopf mit Feinwurzeln im Kanal (wird von unten abgefräst) und Wurzeln im Erdreich; Wiegen per Shader |
