...
export function groundGeometry(R,fill=1,soil=false,section=false){
 const positions=[],uv=[],indices=[],stride=segments+1,layer=(rows+1)*stride;
 ...
 for(let side=0;side<2;side++)for(let j=0;j<=rows;j++)for(let i=0;i<=segments;i++){
  ...
  const r=side?THREE.MathUtils.lerp(inner,envelope,soil?1:progress):inner;
 ...
 if(section){
  ...
  for(const j of [0,rows])for(let i=0;i<segments;i++){
   ...
   for(const source of [a,a+1,a+layer,a+1,a+layer+1,a+layer]){
    ...
    indices.push(positions.length/3);positions.push(x,y,z);uv.push(x/85,z/85);
   }
  }
 }
...
export class GroundGrout{
 constructor(R,soilTextures,mortarTextures){
  ...
  this.soilMaterial=new THREE.MeshStandardMaterial({color:'#3d2a19',...soilTextures,bumpScale:1.6,roughness:1,side:THREE.DoubleSide});
