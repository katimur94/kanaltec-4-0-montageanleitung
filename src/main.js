...
function updateSections(){const options={pipe:$('cutPipe').checked,shield:$('cutShield').checked,holder:$('hideHolder').checked,hideBladder:$('hideBladder').checked};viewer?.setSections(options);$('schematic').textContent=[options.pipe?'Rohr im Schnitt':'Rohr vollständig',options.shield?'Schalung im Schnitt':'Schalung vollständig',...(options.holder?['Halterung ausgeblendet']:[])].join(' · ');try{localStorage.setItem('kanaltec-visibility',JSON.stringify(options));}catch{}}
try{const saved=JSON.parse(localStorage.getItem('kanaltec-visibility'));if(saved)for(const[id,key]of [['cutPipe','pipe'],['cutShield','shield'],['hideHolder','holder'],['hideBladder','hideBladder']])if(typeof saved[key]==='boolean')$(id).checked=saved[key];}catch{}
updateSections();
for(const id of ['cutPipe','cutShield','hideHolder','hideBladder'])$(id).onchange=updateSections;
...
$$('[data-view]').forEach(b=>b.onclick=()=>{viewer?.fit(b.dataset.view);$$('[data-view]').forEach(x=>x.classList.toggle('active',x===b));});
