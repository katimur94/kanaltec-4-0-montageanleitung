...
export class Robot{
 constructor(pipeRadius,anchor){
  ...
  // Longitudinal rotary axis directly behind CutterCam. The complete front
  // assembly rolls here; the lift links have no lateral/yaw joint.
  this.rotationAxis=V(-397,this.bodyY,0);
  this.frontRotation=new THREE.Group();this.frontRotation.name='Drehachse hinter CutterCam';this.frontRotation.position.copy(this.rotationAxis);this.group.add(this.frontRotation);
  this.front=new THREE.Group();this.front.position.copy(this.rotationAxis).negate();this.frontRotation.add(this.front);
  this.frontBase=new THREE.Group();this.front.add(this.frontBase);
 ...
 buildCamera(){
  ...
  const m=this.m,y=this.bodyY,g=new THREE.Group();this.cameraHead=g;g.name='CutterCam · Schwenkkopf, vier LED und Reinigung';g.position.set(-306,y+36,0);this.frontBase.add(g);
  ...
  const coil=[];for(let i=0;i<=420;i++){const t=i/420,a=t*TAU*7;coil.push(V(-401+t*74,y+49+13*Math.cos(a),-43+10*Math.sin(a)));}this.frontBase.add(tube(coil,2.2,m.black,220));
 }
 buildArm(){
  ...
  root.add(box(57,11,94,m.steel,V(-333,y-70,0),2));bake(root);this.frontBase.add(root);
  for(const side of [-1,1]){
   ...
   bake(g);this.front.add(g);this.links.push({o:g,side,lower:false});
   ...
   bake(lower);this.front.add(lower);this.links.push({o:lower,side,lower:true});
  }
  this.actuators=[];for(const side of [-1,1]){const body=cyl(10,125,m.steel),shaft=cyl(5,1,m.polished),capA=cyl(12,12,m.dark),capB=cyl(11,7,m.dark);this.front.add(body,shaft,capA,capB);this.actuators.push({body,shaft,capA,capB,side});}
  this.receiver=new THREE.Group();this.receiver.name='Vierte Achse · Werkzeugaufnahme mit Klappvorrichtung';this.front.add(this.receiver);
 ...
 pose(lift,travel,roll=0,reach=0){
