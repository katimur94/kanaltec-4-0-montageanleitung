...
export const injectionSpec={hoseRadius:5.5,boreRadius:5,coreRadius:4.2};
...
export function injectionFitting(){
 ...
 const body=new THREE.Mesh(new THREE.TubeGeometry(curve,80,7.5,16,false),brass);group.add(body);
 function collar(radius,length,centre,axis,material=brass,sides=48){
  ...
  const hole=new THREE.Path();hole.absarc(0,0,injectionSpec.boreRadius,0,Math.PI*2,true);shape.holes.push(hole);
 ...
 for(const y of [-.5,-2,-3.5])collar(7.85,.45,V(0,y),V(0,1,0),dark);
 collar(9.1,8,bendEnd.clone().addScaledVector(direction,7),direction,brass,6);
 collar(7.2,7,bendEnd.clone().addScaledVector(direction,14.5),direction);
 for(const d of [12,14,16])collar(7.65,.8,bendEnd.clone().addScaledVector(direction,d),direction,dark);
